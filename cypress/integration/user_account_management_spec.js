describe('Admin can add/update/delete doctor/patient or their information', () => {
    it('Admin can add a doctor', () => {
        ...
        cy.findByRole('button', {name: /add doctor/i}).click() //click on "add doctor" tab
    ...
    it('Admin can update a doctor\'s information', () => {
        ...
        }).within(()=>{return cy.findByRole('button', {
            name: /update/i
        }).click()}); //find the update button for user with email : test3@test.com, and click on it
        cy.findByTestId('phone-number').within(()=>{return cy.findByRole('textbox').click()}).clear().type('731-872-4321'); //find the phone field, click on it, clear it, and type new phone number in it
    ...
    it('Admin can add a patient', () => {
        ...
        cy.findByRole('button', {name: /add patient/i}).click() //click on "add doctor" tab
            cy.findByRole('textbox', {
                name: /this\.example@email\.com/i
            }).type('test3@test.com') //enter email
        cy.findByRole('textbox', {
        ...
        cy.findByTestId('ramqNumber').within(()=>{return cy.findByRole('textbox', {
            name: /0000000/i
        }).type('1234567')})  //enter ramq number
        cy.findByTestId('sign-up-psw1').type('Testtest1!') //enter a password
        cy.findByTestId('sign-up-psw2').type('Testtest1!') //confirm password
        cy.findByRole('button', {
            name: /register/i
    ...
    it('Admin can update a patient\'s information', () => {
        ...
        cy.findByRole('checkbox', {
            ...
            name: /update/i
        }).click()}); //find the update button for user with email : jud59@hotmail.com, and click on it
        cy.findByRole('checkbox', {
